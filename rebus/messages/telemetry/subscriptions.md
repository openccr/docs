...
[identity generation](../../discovery/identity.md) for that source node ID. `source_node_id` MUST be in
`0x01–0x7F`, and `context` MUST be valid under the
[telemetry registry](registries.md). A consumer that needs several streams or
...
3. Apply the [profile frame gate](../../profile.md), telemetry payload
   validation, and per-stream sequence processing before evaluating any
   subscription. For each accepted frame, deliver its value and status only to
   active selectors that exactly match its stream, context, and current
   identity generation.
